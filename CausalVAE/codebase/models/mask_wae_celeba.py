...
class CausalWAE(nn.Module):
    def __init__(self, nn='mask', name='vae', z_dim=128, z1_dim=4, z2_dim=32, inference = False):
        ...
        ) for j in range(self.z1_dim)] # z1 = g_1(u)
    ...
    def negative_elbo_bound(self, x, label, mask = None, sample = False, adj = None, beta=1., gamma=1.,
                            lamb1=100., lamb2=100.):
        """
        ...
        # decode_m: z = g1(u) / decode_v=1 ; deterministic
        decode_m = torch.cat([self.z_u[j](label[:, j].to(device)) for j in range(self.z1_dim)], dim=1).reshape([q_m.size()[0], self.z1_dim, self.z2_dim])
        decode_v = torch.ones(q_m.size()[0], self.z1_dim, self.z2_dim).to(device)
        if sample == False:
...
#                                           q_m.reshape([q_m.size()[0], self.z1_dim,self.z2_dim]).to(device))[0]          
        ...
        true_e = torch.randn_like(q_m.view(-1, self.z_dim)) # N x z_dim
        mmd = mmd_penalty(q_m.view(-1, self.z_dim).to(device), true_e.to(device))
...
#             mask_kl = mask_kl + 1*ut.kl_normal(f_z1[:,i,:].to(device), cp_v[:,i,:].to(device), cp_m[:,i,:].to(device), cp_v[:,i,:].to(device))
        ...
        mask_l = beta*l_u + gamma*torch.mean(mask_kl)
        loss = rec + lamb1*mmd + lamb2*hsic + mask_l

        return loss, lamb1*mmd + lamb2*hsic, rec, recon.reshape(x.size()), torch.mean(mask_kl), l_u, f_z1
