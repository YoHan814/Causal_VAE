...
parser.add_argument('--epoch_max',   type=int, default=45,    help="Number of training epochs")
...
if not os.path.exists(f'./figs_vae/run_{args.run:04d}'): 
    os.makedirs(f'./figs_vae/run_{args.run:04d}')
...
for epoch in range(1, args.epoch_max+1):
    ...
    grid = make_grid(torch.cat((u[:32], torch.sigmoid(reconstructed_image[:32])), dim=0), normalize=True)
    save_image(grid, f'figs_vae/run_{args.run:04d}/recon_{epoch}.png')
...
np.save(f'./figs_vae/run_{args.run:04d}/losses.npy',
