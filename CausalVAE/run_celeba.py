...
for epoch in range(1, args.epoch_max+1):
    ...
    grid = make_grid(torch.cat(((u[:32]+1.)/2., torch.sigmoid(reconstructed_image[:32])), dim=0), normalize=True)
    ...
    with torch.no_grad():
        ...
        grid = make_grid(torch.cat(((u[:32]+1.)/2., torch.sigmoid(recon[:32])), dim=0), normalize=True)
